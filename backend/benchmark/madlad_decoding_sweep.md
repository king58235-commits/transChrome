...
Follow-up: MADLAD also lists alternatives separated by spaces ("停下來 停住 停止", "樂器大河 音樂大河"). Chinese puts no spaces between Han characters, so `drop_repeated_clauses` first drops space-separated segments that share half or more of their characters with what was already kept. Across all 338 outputs of the live logs plus both benchmark sets, 15 had a space between Han characters: 12 were such lists and got trimmed, and the 2 real ones ("還有鬧鐘啊 京子的鬧鐘…", "他說 我不知道") were kept. Benchmark sets: only C06/C07/C12 changed; CASE1/4/5 unchanged.
